...
public class TaskDetailFragment extends Fragment {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.it_ok && task == null) {
            ...
            AddNewTaskServices addNewTaskServices = NetRetrofit.instance.getRetrofit().create(AddNewTaskServices.class);
            String token = SharePrefs.getInstance().getAccessToken();
            Call<Task> addNewTask = addNewTaskServices.addTask("application/json", "JWT " + token, newTask);
            addNewTask.enqueue(new Callback<Task>() {
                @Override
